            ...
            .await
        {
            ...
            Err(err) => {
                if start.elapsed() >= max_wait {
                    return Err(std::io::Error::other(format!(
                        "device auth token request timed out after network errors: {err}; debug: {err:?}"
                    )));
                }
                tracing::warn!(
                    error = %err,
                    "device auth token request failed; retrying"
                );
        ...
        if status.is_success() {
            ...
            return serde_json::from_str(&body).map_err(|err| {
                std::io::Error::other(format!(
                    "device auth token response JSON decode failed: {err}; headers: {}; body: {}",
                    truncate_for_error(&headers),
