...
fn virtualize_child_env_replaces_supported_credentials() {
    ...
    let github_token = "fake-github-token-for-tests";
    let openai_api_key = "fake-openai-key-for-tests";
    let mut env = env_map([
    ...
    assert_credential_shape(openai_api_key, openai_dummy, "fake-openai-key-for-tests");
    env.insert("OPENAI_API_KEY".to_string(), "fake-openai-key-for-tests".to_string());
    assert_eq!(
...
fn virtualize_child_env_uses_fresh_dummy_capabilities() {
    let mut first_env = env_map([("OPENAI_API_KEY", "fake-openai-key-for-tests")]);
...
fn virtualize_child_env_preserves_unbound_enterprise_token() {
    ...
    broker.virtualize_child_env(&mut env);
    let inert_token = "fake-github-token-for-tests";
...
fn inject_request_headers_requires_dummy_and_preserves_explicit_authorization() {
    let broker = CredentialBroker::new(/*enabled*/ true);
    let mut env = env_map([("OPENAI_API_KEY", "fake-openai-key-for-tests")]);
    broker.virtualize_child_env(&mut env);
    ...
    broker.inject_request_headers("api.openai.com", &mut headers);
    assert_eq!(authorization(&headers), Some("Bearer fake-openai-key-for-tests"));

    let mut explicit_headers = headers_with_bearer("fake-openai-key-for-tests");
    broker.inject_request_headers("api.openai.com", &mut explicit_headers);

    assert_eq!(authorization(&explicit_headers), Some("Bearer fake-openai-key-for-tests"));
}
