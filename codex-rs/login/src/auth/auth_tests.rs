...
fn login_with_api_key_overwrites_existing_auth_json() {
    ...
    assert_eq!(auth.openai_api_key.as_deref(), Some("fake-openai-key-for-tests"));
...
async fn stored_agent_identity_jwt_keeps_auth_json_unchanged() -> anyhow::Result<()> {
    ...
    Mock::given(method("GET"))
        ...
        .mount(&server)
        .await;
    mock_agent_task_registration(&server, "", &record.agent_runtime_id, "tafake-openai-key-for-tests").await;
    let authapi_base_url = server.uri();
    let chatgpt_base_url = format!("{authapi_base_url}/backend-api");
    save_auth(
        codex_home.path(),
    ...
    assert_eq!(agent_identity_auth.run_task_id(), "tafake-openai-key-for-tests");
...
async fn chatgpt_auth_registers_agent_identity_when_enabled() -> anyhow::Result<()> {
    ...
    Mock::given(method("POST"))
        ...
        .mount(&server)
        .await;
    mock_agent_task_registration(&server, "", "agent-runtime-123", "tafake-openai-key-for-tests").await;
    ...
    assert_eq!(agent_auth.run_task_id(), "tafake-openai-key-for-tests");
    assert_eq!(reused.run_task_id(), "tafake-openai-key-for-tests");
    ...
    assert_eq!(agent_auth.record().task_id.as_deref(), Some("tafake-openai-key-for-tests"));
    assert_eq!(reused.record().task_id.as_deref(), Some("tafake-openai-key-for-tests"));
    let persisted = auth
    ...
    assert_eq!(persisted.task_id.as_deref(), Some("tafake-openai-key-for-tests"));
    ...
    assert_eq!(reloaded_agent_auth.run_task_id(), "tafake-openai-key-for-tests");
    Ok(())
}
...
async fn chatgpt_auth_retries_transient_agent_identity_registration() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        Some("tafake-openai-key-for-tests".to_string())
    );
    Ok(())
}
...
async fn loads_api_key_from_auth_json() {
    ...
    std::fs::write(
        auth_file,
        r#"{"OPENAI_API_KEY":"fake-openai-key-for-tests","tokens":null,"last_refresh":null}"#,
    )
    .unwrap();
    ...
    assert_eq!(auth.api_key(), Some("fake-openai-key-for-tests"));
...
fn logout_removes_auth_file() -> Result<(), std::io::Error> {
    ...
    let auth_dot_json = AuthDotJson {
        ...
        openai_api_key: Some("fake-openai-key-for-tests".to_string()),
        tokens: None,
        last_refresh: None,
        agent_identity: None,
        personal_access_token: None,
...
async fn load_auth_reads_access_token_from_env() {
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(ResponseTemplate::new(200).set_body_json(json!({
            "task_id": "tafake-openai-key-for-tests",
        })))
        .expect(1)
        .mount(&server)
        .await;
    expected_record.task_id = Some("tafake-openai-key-for-tests".to_string());
    ...
    assert_eq!(agent_identity.run_task_id(), "tafake-openai-key-for-tests");
    assert!(
...
async fn load_auth_keeps_codex_api_key_env_precedence() {
    ...
    let agent_identity = fake_agent_identity_jwt(&record).expect("fake agent identity");
    let _access_token_guard = EnvVarGuard::set(CODEX_ACCESS_TOKEN_ENV_VAR, &agent_identity);
    let _api_key_guard = EnvVarGuard::set(CODEX_API_KEY_ENV_VAR, "fake-openai-key-for-tests");
...
async fn enforce_login_restrictions_blocks_env_api_key_when_chatgpt_required() {
    let _guard = EnvVarGuard::set(CODEX_API_KEY_ENV_VAR, "fake-openai-key-for-tests");
    let _access_token_guard = remove_access_token_env_var();
    let codex_home = tempdir().unwrap();
...
const TEST_AGENT_IDENTITY_RSA_PRIVATE_KEY_PEM: &[u8] = br#"-----BEGIN TEST PRIVATE KEY-----
